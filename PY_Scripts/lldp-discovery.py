...
All IFD's have an IFL configured.

Algorithm :

Configure all devices in the list with default LLDP configs and commit.
Take "show lldp neighbors" from all routers.
Parse the output and draw a topology, with parent i/f and port info.
    ...
    try:
        ...
        if user_choice == "1":
            ...
            for each_dict in final_devices_list:
                ...
                print "OSPF Neighbors by ID: " + Fore.YELLOW + Style.BRIGHT + "%s" % ', '.join(
                    each_dict["NbrRtrId"]) + Fore.BLUE + Style.BRIGHT
                print "OSPF Neighbors by IP: " + Fore.YELLOW + Style.BRIGHT + "%s" % ', '.join(
                    each_dict["NbrRtrIp"]) + Fore.BLUE + Style.BRIGHT
                print "\n"
        ...
        # Printing devices to CSV file
            ...
            for each_dict in final_devices_list:
                print >> csv_file, each_dict["Host"] + ";" + each_dict["HostId"] + ";" + ', '.join(
                    each_dict["NbrRtrId"]) + ";" + ', '.join(each_dict["NbrRtrIp"])
