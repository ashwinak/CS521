...
try:
    ...
    os.system('rm -rf ./jtimon0712-' + sys.argv[1])
    os.system('cp jtimon0712 jtimon0712-'+sys.argv[1])
    #os.system('echo \'lab1234\' | sudo -S nohup ./jtimon0712-'+sys.argv[1]+ ' --config ' + sys.argv[1] + '.json  --log /home/ashwinak/grpc/gtc/log/' + sys.argv[1] + '.log &')
    ...
    os.system('echo \'lab1234\' | sudo -S nohup ./jtimon0712-' +sys.argv[1]+ ' --config ' + sys.argv[1] + '.json &')
    while True:
        ...
        stat2 = stat2.strip()
        if stat2 != stat1:
            print "%%%Session already active,do nothing"
        else:
