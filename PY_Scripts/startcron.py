#root@6525eb3b5331:/# more /opt/open-nti/startcron.py
#!/usr/bin/env python
#
...
if (args.action == "add"):
    ...
    if (re.search(time_sec_regex, args.time)):
        time = int(re.sub(r'\D', "", args.time))
    elif (re.search(time_min_regex, args.time)):
        time = int(re.sub(r'\D', "", args.time))
        if (time < 1 or time > 59):
            print "Incorrect -t value " + str(time) + " for minutes. Value must be between 1 and 59."
            exit()
    else:
        print "Incorrect -t format " + str(args.time) + ". Format: -t 30s or 10m or 1h"
        exit()
